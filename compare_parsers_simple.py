...
def parse_with_both(url: str) -> Dict[str, Any]:
    ...
    print(
        f"  ⏱️  Old parser: {old_duration:.2f}s, New parser: {new_duration:.2f}s"
    )
...
def compare_results(old_result: Dict, new_result: Dict) -> Dict[str, Any]:
    ...
    if old_norm != new_norm:
        comparison["match"] = False
        comparison["differences"].append(
            "description differs after normalization"
        )
    ...
    for field in ["authors", "tags", "urls"]:
        ...
        if len(old_val) != len(new_val):
            comparison["match"] = False
            comparison["differences"].append(
                f"{field} count: {len(old_val)} != {len(new_val)}"
            )

    return comparison


def save_mismatch_detail(
    url: str, comparison: Dict, mismatch_count: int
) -> None:
...
def main():
    ...
    parser.add_argument(
        "--max-urls", type=int, default=25, help="Maximum URLs to test"
    )
    parser.add_argument(
        "--start-from", type=int, default=0, help="Start from URL index"
    )
    ...
    mismatch_rate = (
        (stats["mismatches"] / stats["successful"]) * 100
    ...
    if stats["mismatches"] > 0:
        ...
        if mismatch_rate < target_rate:
            print(
                f"✅ SUCCESS: Mismatch rate {mismatch_rate:.1f}% is < target {target_rate}%"
            )
        else:
            print(
                f"⚠️  NEEDS WORK: Mismatch rate {mismatch_rate:.1f}% exceeds target {target_rate}%"
            )
