...
QSP_DETAILS = re.compile(
    r'<table class="sobi2Details"[^>]*>(.*?)</table>', re.DOTALL
)
QSP_TR = re.compile(r"<tr>(.*?)</tr>", re.DOTALL)
QSP_TITLE = re.compile(r"<h1>(.*?)</h1>")
QSP_IMG = re.compile(r'<img src="([^"]+)"[^>]*class="sobi2DetailsImage"')
QSP_FIELD = re.compile(
    r'<span\s+id="sobi2Details_field_([^"]+)"\s*>'
    "(?:<span .*?</span> )?(.*?)</span>",
    re.DOTALL,
)
QSP_DOWNLOAD_LINK = re.compile(
    r'<h2><a href="([^"]+)" title="download">'
    ...
    if m:
        ...
        if n:
            res["release_date"] = datetime.datetime(
                int(n.group(3)), int(n.group(2)), int(n.group(1))
            ).date()
