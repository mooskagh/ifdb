...
from games.models import URL, GameURL, InterpretedGameUrl
...
def RecodeGame(game_url_id):
    ...
    logger.info(
        "GamUrl %d, Game %d [%s], Url %d: %s"
        % (
            game_url_id,
            game_url.game_id,
            game_url.game.title,
            game_url.url_id,
            game_url.url.original_url,
        )
    )
    metadata = {
    ...
    if configuration.get("interpreter") != "urqw":
        logger.error(
            "Recoding for unknown interpreter: %s"
    ...
    try:
        subprocess.check_output(
            ...
            % (url.GetFs().path(url.local_filename), tmp_dir),
    ...
    with zipfile.ZipFile(
        fs.open(new_filename, "wb"), "w", zipfile.ZIP_DEFLATED, allowZip64=True
    ) as z:
